// src/components/room/RoomReviews.tsx
"use client";
...
export default function RoomReviews({ listingId }: RoomReviewsProps) {
  ...
  const ratingCategories = [
    { name: "Cleanliness", icon: <SprayCan className="h-6 w-6" /> },
    { name: "Accuracy", icon: <CheckCircle className="h-6 w-6" /> },
    { name: "Check-in", icon: <Key className="h-6 w-6" /> },
    { name: "Communication", icon: <MessageSquare className="h-6 w-6" /> },
    { name: "Location", icon: <Map className="h-6 w-6" /> },
    { name: "Value", icon: <Tag className="h-6 w-6" /> },
  ];
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
          {ratingCategories.map((category) => {
            const avgRating =
              reviews.reduce(
                (acc, review) =>
                  acc + review.categories[category.name.toLowerCase() as keyof typeof review.categories],
                0
              ) / reviews.length;
            ...
            return (
              <div
                key={category.name}
                className="flex flex-col items-start py-4 px-4 border rounded-lg shadow-sm bg-card/20"
              >
                <p className="font-medium">{category.name}</p>
                <p className="font-medium">{avgRating.toFixed(1)}</p>
                <div className="mt-2">{category.icon}</div>
              </div>
            );
          })}
        </div>
