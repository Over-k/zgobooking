...
export default function NewListingPage() {
    ...
    try {
      const response = await fetch('/api/listings', {
        ...
        body: JSON.stringify({
          ...
          maxAdults: parseInt(values.maxAdults),
          maxChildren: parseInt(values.maxChildren),
          maxInfants: parseInt(values.maxInfants),
          maxPets: parseInt(values.maxPets),
          minimumStay: parseInt(values.minimumStay),
          maximumStay: parseInt(values.maximumStay),
  ...
  const nextStep = async () => {
    ...
    if (currentStep === 1 && selectedFiles.length > 0) {
      ...
      try {
        ...
        const mapped = images.map((img: { url: string }, idx: number) => ({
          url: img.url,
          isPrimary: idx === 0,
          caption: "",
        }));
  ...
  return (
    <div className="space-y-6">
      <div>
        <Breadcrumb />
        <h1 className="mt-2 text-3xl font-bold">Add New Listing</h1>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {
              [
                "Add the basic details about your listing",
                "Upload photos of your property",
                "Select the amenities available in your property",
                "Set house rules and safety features",
                "Set available dates and cancellation policy",
              ][currentStep]
            }
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form form={form} onSubmit={onSubmit}>
            <div className="space-y-6">
              ...
              {currentStep === 1 && (
                <div className="space-y-6">
                  <FileUploadComponent selectedFiles={selectedFiles} setSelectedFiles={setSelectedFiles} />
                        ...
                        {AMENITY_OPTIONS[
                          ...
                          return (
                            <FormField
                              key={option}
                              control={form.control}
                              name={`amenities.${category}` as "amenities.essential" | "amenities.safety" | "amenities.outdoor" | "amenities.features" | "amenities.accessibility"}
                              render={({ field }) => (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                      <div className="text-sm text-muted-foreground">
                        Select the dates when your property will be available for booking. You can select multiple date ranges.
                      </div>
                      <FormField
                        control={form.control}
                        name="availableDates"
                        render={({ field }) => (
                          <FormItem>
                            ...
                            {field.value && field.value.length > 0 && (
                              <div className="mt-4">
                                <p className="text-sm font-medium mb-2">Selected dates ({field.value.length}):</p>
                                <div className="text-sm text-muted-foreground">
                                  {field.value.slice(0, 5).map((date, index) => (
                                    <span key={index} className="inline-block bg-muted px-2 py-1 rounded mr-2 mb-1">
